...
test("POST /api/calendar/end-of-day-rollover archives today's unfinished task blocks and reschedules tomorrow", async (t) => {
  ...
  process.env.DATABASE_URL = `pg-mem://focus-exec-calendar-${Date.now()}-${Math.random()}`;
  ...
  delete require.cache[require.resolve("../services/taskBlockRecovery")];
  ...
  `, [missedBlockId, "test-user-id", localTimestamp(todayKey, 15, 0), localTimestamp(todayKey, 15, 30), taskOneId]);
  await dbRun(`
    INSERT INTO calendar_events
      (id, user_id, title, start_time, end_time, event_type, task_id, block_state, color, synced_at)
    VALUES (?, ?, 'Ship fix', ?, ?, 'task_block', ?, 'scheduled', 'blue', CURRENT_TIMESTAMP)
  `, [scheduledBlockId, "test-user-id", localTimestamp(todayKey, 22, 30), localTimestamp(todayKey, 23, 0), taskTwoId]);

  const response = await fetch(`http://127.0.0.1:${server.address().port}/api/calendar/end-of-day-rollover`, {
    method: "POST",
  });
