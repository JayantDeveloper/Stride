...
router.get("/", async (req, res) => {
  let query = "SELECT * FROM accountability_checkins WHERE user_id = ?";
  const params = [req.user.id];

  if (req.query.date) {
    query += " AND date(prompted_at) = date(?)";
...
router.post("/", async (req, res) => {
  ...
  await dbRun(`
    INSERT INTO accountability_checkins
      (id, user_id, focus_session_id, task_id, outcome, notes, ai_followup, completed_at)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `, [checkin.id, checkin.user_id, checkin.focus_session_id, checkin.task_id, checkin.outcome,
      checkin.notes, checkin.ai_followup, checkin.completed_at]);

  const created = await dbGet(
    "SELECT * FROM accountability_checkins WHERE id = ? AND user_id = ?",
    [id, req.user.id]
  );
...
router.patch("/:id", async (req, res) => {
  ...
  if (req.body.outcome !== undefined) {
    updates.push("outcome = ?", "completed_at = CURRENT_TIMESTAMP");
  ...
  await dbRun(`UPDATE accountability_checkins SET ${updates.join(", ")} WHERE id = ? AND user_id = ?`, params);
