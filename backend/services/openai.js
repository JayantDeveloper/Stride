...
async function suggestSlots({ task, freeSlots }) {
  const system =
    "You are a productivity scheduling assistant. Suggest the best time slots for focused work sessions. Bias toward: harder/higher-priority tasks earlier in the day, avoiding back-to-back blocks without buffer. Be direct and brief.";
...
async function generateDayPlan({ date, tasks, events, morningNote = "" }) {
  const system =
    "You are a productivity scheduling assistant creating a realistic daily plan. Keep it executable, not aspirational. Prioritize Urgent and High priority tasks. Leave buffer time between blocks. Respect existing calendar events.";
...
async function generateCheckinResponse({ taskTitle, outcome, notes = "" }) {
  const system =
    "You are a supportive but direct productivity coach. Respond to focus session check-ins with 2-3 sentences. Be action-oriented. If blocked: suggest one concrete unblocking step. If skipped: non-judgmental redirect. If partial: affirm progress and clarify what's left. No filler or fluff.";
...
async function organizeCalendar({
  date,
  tasks,
  existingEvents,
  freeSlots,
  ...
  const strategyInstructions = {
    priority:
      "Order tasks by priority: Urgent → High → Medium → Low. Within same priority, schedule shorter tasks first.",
    shortest_first:
      "Order tasks by estimated duration, shortest first. This maximizes the number of tasks completed.",
    hardest_first:
      "Schedule high-difficulty and high-priority tasks in the morning (before noon) when cognitive energy is highest. Schedule easier, lower-priority tasks in the afternoon.",
    manual_order:
      "Schedule tasks in the exact order provided (already sorted by user's manual list order). Do not reorder.",
    ai_decide:
      "You decide the optimal order. Consider: priority, difficulty, estimated duration, time of day (hard tasks earlier), and available slot sizes. Explain your reasoning in the summary.",
  };

  const instruction =
    strategyInstructions[strategy] ?? strategyInstructions.priority;
  ...
  const eventsText = existingEvents.length
    ? existingEvents
        .map(
          (event) =>
            `  - ${formatTime(event.start_time)}–${formatTime(event.end_time)}: "${event.title}" (BLOCKED, cannot move)`,
        )
        .join("\n")
    : "  None";
...
async function generateTaskBreakdown({ taskTitle, notes = "", context = "" }) {
  const system =
    "You turn a task into 3 to 5 concrete execution steps. Every step must be action-oriented, specific, and small enough to start immediately. Avoid vague advice. Return ONLY valid JSON.";
...
async function generateTaskNextStep({
  taskTitle,
  notes = "",
  context = "",
  recentState = {},
  ...
  const system =
    "You choose exactly one small executable next action for a task. It must be concrete, specific, and phrased as something the user can do now. Return ONLY valid JSON.";
...
async function generateSprintGoal({
  ...
  const system =
    "You write exactly one sentence describing the goal for the next focus sprint. It should be concrete, narrow, and achievable within a single sprint. Return ONLY valid JSON.";
...
async function generateEveningReview({
  ...
  const system =
    "You are a productivity coach writing a brief end-of-day review. Be honest, specific, and constructive. Acknowledge real wins, identify one pattern, suggest one thing for tomorrow. 3-4 sentences max. No generic platitudes.";

  const totalFocusMins = sessions.reduce(
    (sum, session) => sum + (session.actual_mins ?? session.planned_mins ?? 0),
    0,
  );
  const outcomeSummary =
    checkins
      .map((checkin) => checkin.outcome)
      .filter(Boolean)
      .join(", ") || "none";
...
${
  sessions
    ...
    .map((session) => `Note: "${session.notes}"`)
    .join("\n") || ""
}
